module.exports = {
id: "Bogdan02Popup",

thumbnailWidth: 564,
thumbnailHeight: 391,
title: "Bogdan02Popup",

keywords: "popup",
cat: [1],
type: 0,
pro: true,
resolve: {
    type: "SectionPopup",
    value: {
        _styles: [
            "sectionPopup"
        ],
        items: [
            {
                type: "Row",
                value: {
                    _styles: [
                        "row"
                    ],
                    items: [
                        {
                            type: "Column",
                            value: {
                                _styles: [
                                    "column"
                                ],
                                items: [
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--spacer"
                                            ],
                                            items: [
                                                {
                                                    type: "Spacer",
                                                    value: {
                                                        _styles: [
                                                            "spacer"
                                                        ],
                                                        height: 50,
                                                        mobileHeight: 50,
                                                        tabletHeight: 71
                                                    }
                                                }
                                            ]
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            items: [
                                                {
                                                    type: "RichText",
                                                    value: {
                                                        _styles: [
                                                            "richText"
                                                        ],
                                                        text: "<p class=\"brz-fs-sm-40 brz-text-lg-center brz-lh-lg-1_3 brz-fs-lg-42 brz-ls-lg-m_1_5 brz-ff-yeseva_one brz-fw-lg-400 brz-fs-xs-26\"><span class=\"brz-cp-color2\">Hey stranger!</span></p>"
                                                    }
                                                }
                                            ],
                                            paddingType: "ungrouped",
                                            paddingTop: 0,
                                            paddingTopSuffix: "px",
                                            padding: 0,
                                            paddingLeft: 0,
                                            paddingLeftSuffix: "px",
                                            paddingBottom: 0,
                                            paddingBottomSuffix: "px",
                                            animationName: "bounceIn",
                                            tempAnimationName: "bounceIn",
                                            animationDuration: 1100,
                                            animationDelay: 200
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            items: [
                                                {
                                                    type: "RichText",
                                                    value: {
                                                        _styles: [
                                                            "richText"
                                                        ],
                                                        text: "<p class=\"brz-fs-xs-19 brz-mb-lg-20 brz-text-lg-center brz-ls-lg-0 brz-ff-average_sans brz-fw-lg-400 brz-lh-lg-1_5 brz-fs-lg-18 brz-fs-sm-20\"><span class=\"brz-cp-color2\">This here is a nice round Newsletter Popup &amp; Special Offer example.</span></p><p class=\"brz-fs-xs-20 brz-fs-sm-20 brz-fw-lg-400 brz-ff-average_sans brz-ls-lg-0 brz-lh-lg-1_7 brz-text-lg-center brz-fs-lg-18 brz-mb-lg-5\"><span class=\"brz-cp-color7\">&nbsp;</span><span class=\"brz-cp-color2\">Sign up today and&nbsp;</span><strong class=\"brz-cp-color2\">get 25% discount</strong><span class=\"brz-cp-color2\"> on your next order with our site.&nbsp;</span></p>"
                                                    }
                                                }
                                            ],
                                            paddingType: "ungrouped",
                                            paddingRight: 20,
                                            paddingRightSuffix: "px",
                                            padding: 0,
                                            paddingLeft: 20,
                                            paddingLeftSuffix: "px"
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--iconText"
                                            ],
                                            items: [
                                                {
                                                    type: "Form",
                                                    value: {
                                                        _styles: [
                                                            "form"
                                                        ],
                                                        items: [
                                                            {
                                                                type: "FormFields",
                                                                value: {
                                                                    items: [
                                                                        {
                                                                            type: "FormField",
                                                                            value: {
                                                                                type: "Text",
                                                                                label: "Email address",
                                                                                required: "off",
                                                                                options: [
                                                                                    "Option 1",
                                                                                    "Option 2"
                                                                                ],
                                                                                width: 100
                                                                            }
                                                                        }
                                                                    ],
                                                                    fontSize: 15,
                                                                    fontFamily: "average_sans",
                                                                    lineHeight: 1.100000000000000088817841970012523233890533447265625,
                                                                    letterSpacing: 0,
                                                                    fontWeight: 400,
                                                                    fontStyle: "",
                                                                    padding: 20,
                                                                    paddingRight: 20,
                                                                    paddingBottom: 20,
                                                                    paddingLeft: 20,
                                                                    mobileFontStyle: "",
                                                                    borderColorPalette: "",
                                                                    borderColorHex: "#8d7566",
                                                                    borderColorOpacity: 0.36999999999999999555910790149937383830547332763671875,
                                                                    colorPalette: "color1",
                                                                    colorHex: "#191b21",
                                                                    colorOpacity: 0.5,
                                                                    mobileFontSize: 16,
                                                                    mobileLineHeight: 1.5,
                                                                    mobileLetterSpacing: 0,
                                                                    mobileFontWeight: 400,
                                                                    tabletFontSize: 17,
                                                                    tabletLineHeight: 1.5,
                                                                    tabletLetterSpacing: 0,
                                                                    tabletFontWeight: 400,
                                                                    tabletFontStyle: ""
                                                                }
                                                            },
                                                            {
                                                                type: "Button",
                                                                value: {
                                                                    _styles: [
                                                                        "button",
                                                                        "submit"
                                                                    ],
                                                                    size: "medium",
                                                                    mobileSize: "medium",
                                                                    fontSize: 15,
                                                                    paddingTB: 14,
                                                                    paddingTop: 14,
                                                                    paddingBottom: 14,
                                                                    tempPaddingTB: 14,
                                                                    tempPaddingTop: 14,
                                                                    tempPaddingBottom: 14,
                                                                    paddingRL: 42,
                                                                    paddingRight: 42,
                                                                    paddingLeft: 42,
                                                                    tempPaddingRL: 42,
                                                                    tempPaddingRight: 42,
                                                                    tempPaddingLeft: 42,
                                                                    borderRadius: 4,
                                                                    borderWidth: 2,
                                                                    mobilePaddingTop: 14,
                                                                    mobilePaddingRight: 42,
                                                                    mobilePaddingBottom: 14,
                                                                    mobilePaddingLeft: 42,
                                                                    text: "SIGN-UP",
                                                                    iconName: "",
                                                                    iconType: "",
                                                                    bgColorPalette: "color2",
                                                                    tempBgColorPalette: "color2",
                                                                    bgColorOpacity: 1,
                                                                    borderRadiusType: "custom",
                                                                    fillType: "filled",
                                                                    borderColorPalette: 0,
                                                                    tempBorderColorPalette: 0,
                                                                    hoverBgColorOpacity: 1,
                                                                    hoverBgColorPalette: "color1",
                                                                    hoverBorderColorPalette: "",
                                                                    tempHoverBorderColorPalette: "",
                                                                    hoverBgColorHex: "#142850",
                                                                    tempHoverBgColorOpacity: 1,
                                                                    tempHoverBgColorPalette: "color3",
                                                                    hoverBorderColorHex: "#142850",
                                                                    hoverBorderColorOpacity: 0.8000000000000000444089209850062616169452667236328125,
                                                                    tempBorderWidth: 2,
                                                                    borderColorOpacity: 0,
                                                                    fontFamily: "yeseva_one",
                                                                    lineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                                    letterSpacing: 3,
                                                                    fontWeight: 400,
                                                                    fontStyle: "",
                                                                    tempBorderRadiusType: "custom",
                                                                    tempBorderRadius: 4,
                                                                    tempMobileBorderRadius: 4,
                                                                    tabletFontSize: 13,
                                                                    tabletLineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                                    tabletLetterSpacing: 3,
                                                                    tabletFontWeight: 600,
                                                                    tabletFontStyle: "",
                                                                    tabletSize: "medium",
                                                                    tabletPaddingTB: 14,
                                                                    tabletPaddingTop: 14,
                                                                    tabletPaddingBottom: 14,
                                                                    tempTabletPaddingTB: 14,
                                                                    tempTabletPaddingTop: 14,
                                                                    tempTabletPaddingBottom: 14,
                                                                    tabletPaddingRL: 42,
                                                                    tabletPaddingRight: 42,
                                                                    tabletPaddingLeft: 42,
                                                                    tempTabletPaddingRL: 42,
                                                                    tempTabletPaddingRight: 42,
                                                                    tempTabletPaddingLeft: 42,
                                                                    tabletBorderRadius: 4,
                                                                    mobileFontSize: 13,
                                                                    mobilePaddingTB: 14,
                                                                    tempMobilePaddingTB: 14,
                                                                    tempMobilePaddingTop: 14,
                                                                    tempMobilePaddingBottom: 14,
                                                                    mobilePaddingRL: 42,
                                                                    tempMobilePaddingRL: 42,
                                                                    tempMobilePaddingRight: 42,
                                                                    tempMobilePaddingLeft: 42,
                                                                    mobileBorderRadius: 4,
                                                                    mobileLineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                                    mobileLetterSpacing: 3,
                                                                    mobileFontWeight: 600,
                                                                    mobileFontStyle: "",
                                                                    iconSpacing: 6
                                                                }
                                                            }
                                                        ],
                                                        horizontalAlign: "center",
                                                        tabletHorizontalAlign: "center"
                                                    }
                                                }
                                            ]
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--spacer"
                                            ],
                                            items: [
                                                {
                                                    type: "Spacer",
                                                    value: {
                                                        _styles: [
                                                            "spacer"
                                                        ],
                                                        height: 50,
                                                        mobileHeight: 50,
                                                        tabletHeight: 71
                                                    }
                                                }
                                            ]
                                        }
                                    }
                                ],
                                width: 100,
                                paddingBottom: 20,
                                paddingBottomSuffix: "px",
                                padding: 20,
                                paddingLeft: 18,
                                paddingLeftSuffix: "%",
                                paddingRight: 18,
                                paddingRightSuffix: "%",
                                paddingTop: 20,
                                paddingTopSuffix: "px",
                                tabletMarginRight: 100,
                                tabletMarginRightSuffix: "px",
                                tabletMargin: 0,
                                tabletMarginLeft: 100,
                                tabletMarginLeftSuffix: "px",
                                tabletMarginType: "ungrouped",
                                bgColorPalette: "",
                                bgColorHex: "#fff443",
                                bgColorOpacity: 1,
                                tabletPaddingRight: 60,
                                tabletPaddingLeft: 60,
                                mobilePaddingRight: 50,
                                mobilePaddingLeft: 50,
                                borderRadius: 300,
                                borderTopLeftRadius: 300,
                                borderTopRightRadius: 300,
                                borderBottomLeftRadius: 300,
                                borderBottomRightRadius: 300,
                                tempBorderRadius: 100,
                                tempBorderTopLeftRadius: 100,
                                tempBorderTopRightRadius: 100,
                                tempBorderBottomRightRadius: 100,
                                tempBorderBottomLeftRadius: 100,
                                boxShadow: "on",
                                boxShadowBlur: 20,
                                boxShadowColorOpacity: 0.179999999999999993338661852249060757458209991455078125,
                                boxShadowSpread: 2,
                                boxShadowVertical: 15,
                                boxShadowColorHex: "#000000",
                                boxShadowColorPalette: "",
                                tabletPaddingType: "ungrouped",
                                tabletPaddingRightSuffix: "px",
                                tabletPadding: 15,
                                tabletPaddingLeftSuffix: "px",
                                mobilePaddingRightSuffix: "px",
                                mobilePadding: 0,
                                mobilePaddingLeftSuffix: "px",
                                mobileMarginType: "grouped",
                                mobileMargin: 0,
                                mobileMarginSuffix: "px",
                                mobileMarginTop: 0,
                                mobileMarginRight: 0,
                                mobileMarginBottom: 0,
                                mobileMarginLeft: 0
                            }
                        }
                    ],
                    bgColorPalette: "",
                    bgColorHex: "#fff443",
                    bgColorOpacity: 0,
                    mobilePaddingRight: 0,
                    mobilePaddingLeft: 0,
                    borderRadius: 0,
                    borderTopLeftRadius: 0,
                    borderTopRightRadius: 0,
                    borderBottomLeftRadius: 0,
                    borderBottomRightRadius: 0,
                    tempBorderRadius: 100,
                    tempBorderTopLeftRadius: 100,
                    tempBorderTopRightRadius: 100,
                    tempBorderBottomRightRadius: 100,
                    tempBorderBottomLeftRadius: 100,
                    tempBgColorOpacity: 1,
                    boxShadow: "off",
                    boxShadowBlur: 51,
                    boxShadowColorOpacity: 0.289999999999999980015985556747182272374629974365234375,
                    boxShadowSpread: -9,
                    boxShadowVertical: 21,
                    boxShadowColorHex: "#5a3b0f",
                    boxShadowColorPalette: "",
                    tabletPaddingRight: 0,
                    tabletPaddingLeft: 0
                }
            }
        ],
        _blockVisibility: "unlisted",
        containerSize: 46,
        bgColorPalette: "color8",
        bgColorHex: "#ffffff",
        bgColorOpacity: 0.90000000000000002220446049250313080847263336181640625,
        tempBgColorOpacity: 0.90000000000000002220446049250313080847263336181640625,
        colorPalette: "color2",
        colorOpacity: 1
    }
}
};