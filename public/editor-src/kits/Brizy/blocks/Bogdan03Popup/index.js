module.exports = {
id: "Bogdan03Popup",

thumbnailWidth: 564,
thumbnailHeight: 391,
title: "Bogdan03Popup",

keywords: "popup",
cat: [1],
type: 0,
pro: true,
resolve: {
    type: "SectionPopup",
    value: {
        _styles: [
            "sectionPopup"
        ],
        items: [
            {
                type: "Row",
                value: {
                    _styles: [
                        "row"
                    ],
                    items: [
                        {
                            type: "Column",
                            value: {
                                _styles: [
                                    "column"
                                ],
                                items: [
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            items: [
                                                {
                                                    type: "RichText",
                                                    value: {
                                                        _styles: [
                                                            "richText"
                                                        ],
                                                        text: "<p class=\"brz-mb-xs-10 brz-fw-xs-800 brz-fs-xs-23 brz-ls-sm-1_5 brz-fs-sm-28 brz-mb-sm-10 brz-mt-sm-10 brz-lh-lg-1_3 brz-fw-lg-700 brz-ff-saira_extra_condensed brz-fs-lg-44 brz-ls-lg-0 brz-text-lg-center brz-mt-lg-0 brz-mb-lg-0 brz-fw-sm-700 brz-ls-xs-1_5\"><span style=\"color: rgb(255, 255, 255);\">END OF SEASON SALE</span></p><p class=\"brz-fs-xs-18 brz-fs-lg-19 brz-fw-lg-400 brz-ff-average_sans brz-ls-lg-0 brz-lh-lg-1_3 brz-text-lg-center brz-fs-sm-18\"><span style=\"color: rgb(0, 6, 7);\">--- Our Mid Top sneakers are now ---</span>&nbsp;</p><p class=\"brz-mt-xs-10 brz-fw-xs-800 brz-fs-sm-70 brz-fs-lg-63 brz-mb-lg-0 brz-fw-lg-900 brz-ff-saira brz-ls-lg-0 brz-lh-lg-1_3 brz-text-lg-center brz-mt-lg-0 brz-fw-sm-800 brz-fs-xs-50\"><span style=\"color: rgb(255, 255, 255);\">30% OFF&nbsp;</span></p>"
                                                    }
                                                }
                                            ],
                                            tabletMarginBottom: 0,
                                            tabletMarginBottomSuffix: "px",
                                            tabletMargin: 0
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--image"
                                            ],
                                            items: [
                                                {
                                                    type: "Image",
                                                    value: {
                                                        _styles: [
                                                            "image"
                                                        ],
                                                        resize: 65,
                                                        imageWidth: 700,
                                                        imageHeight: 287,
                                                        imageSrc: "c88d5afe4807f0c392f3e001042854c4.png",
                                                        height: 100,
                                                        positionX: 50,
                                                        positionY: 50,
                                                        imagePopulation: "",
                                                        mobilePositionX: 52,
                                                        mobilePositionY: 100,
                                                        mobileHeight: 84,
                                                        mobileResize: 100,
                                                        tabletResize: 65,
                                                        tabletHeight: 90,
                                                        tabletPositionX: 52,
                                                        tabletPositionY: 100
                                                    }
                                                }
                                            ],
                                            marginType: "grouped"
                                        }
                                    },
                                    {
                                        type: "Cloneable",
                                        value: {
                                            _styles: [
                                                "wrapper-clone",
                                                "wrapper-clone--button"
                                            ],
                                            items: [
                                                {
                                                    type: "Button",
                                                    value: {
                                                        _styles: [
                                                            "button"
                                                        ],
                                                        bgColorPalette: "color8",
                                                        tempBgColorPalette: "color8",
                                                        bgColorOpacity: 1,
                                                        borderRadiusType: "custom",
                                                        fillType: "filled",
                                                        borderColorPalette: 0,
                                                        tempBorderColorPalette: "color8",
                                                        hoverBgColorOpacity: 0.91000000000000003108624468950438313186168670654296875,
                                                        colorHex: "#0bb9c0",
                                                        colorOpacity: 1,
                                                        colorPalette: "",
                                                        tempColorOpacity: 1,
                                                        hoverColorHex: "#ffffff",
                                                        hoverColorOpacity: 1,
                                                        text: "SHOP NOW",
                                                        fontSize: 21,
                                                        fontFamily: "saira_extra_condensed",
                                                        lineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                        letterSpacing: 3,
                                                        fontWeight: 700,
                                                        fontStyle: "",
                                                        borderRadius: 4,
                                                        iconName: "triangle-right-17",
                                                        iconType: "glyph",
                                                        tempBorderRadiusType: "custom",
                                                        paddingRight: 42,
                                                        paddingLeft: 42,
                                                        borderWidth: 0,
                                                        borderColorOpacity: 0,
                                                        hoverBorderColorOpacity: 0,
                                                        mobileBorderRadius: 4,
                                                        tempBorderRadius: 4,
                                                        tempMobileBorderRadius: 4,
                                                        boxShadow: "on",
                                                        boxShadowColorHex: "#053433",
                                                        boxShadowColorOpacity: 1,
                                                        boxShadowColorPalette: "",
                                                        boxShadowSpread: -6,
                                                        boxShadowVertical: 9,
                                                        boxShadowBlur: 24,
                                                        hoverBgColorPalette: "",
                                                        hoverBorderColorPalette: "",
                                                        tempHoverBorderColorPalette: "",
                                                        hoverBgColorHex: "#010809",
                                                        tempHoverBgColorOpacity: 0.91000000000000003108624468950438313186168670654296875,
                                                        tempHoverBgColorPalette: "",
                                                        hoverBorderColorHex: "#010809",
                                                        tempBorderWidth: 2,
                                                        tabletSize: "medium",
                                                        tabletFontSize: 20,
                                                        tabletPaddingTB: 14,
                                                        tabletPaddingTop: 14,
                                                        tabletPaddingBottom: 14,
                                                        tempTabletPaddingTB: 14,
                                                        tempTabletPaddingTop: 14,
                                                        tempTabletPaddingBottom: 14,
                                                        tabletPaddingRL: 42,
                                                        tabletPaddingRight: 42,
                                                        tabletPaddingLeft: 42,
                                                        tempTabletPaddingRL: 42,
                                                        tempTabletPaddingRight: 42,
                                                        tempTabletPaddingLeft: 42,
                                                        tabletBorderRadius: 4,
                                                        tabletLineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                        tabletLetterSpacing: 3,
                                                        tabletFontWeight: 600,
                                                        tabletFontStyle: "",
                                                        mobileSize: "medium",
                                                        mobileFontSize: 20,
                                                        mobilePaddingTB: 14,
                                                        mobilePaddingTop: 14,
                                                        mobilePaddingBottom: 14,
                                                        tempMobilePaddingTB: 14,
                                                        tempMobilePaddingTop: 14,
                                                        tempMobilePaddingBottom: 14,
                                                        mobilePaddingRL: 42,
                                                        mobilePaddingRight: 42,
                                                        mobilePaddingLeft: 42,
                                                        tempMobilePaddingRL: 42,
                                                        tempMobilePaddingRight: 42,
                                                        tempMobilePaddingLeft: 42,
                                                        mobileLineHeight: 1.8000000000000000444089209850062616169452667236328125,
                                                        mobileLetterSpacing: 3,
                                                        mobileFontWeight: 600,
                                                        mobileFontStyle: ""
                                                    }
                                                }
                                            ]
                                        }
                                    },
                                    {
                                        type: "Wrapper",
                                        value: {
                                            _styles: [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            items: [
                                                {
                                                    type: "RichText",
                                                    value: {
                                                        _styles: [
                                                            "richText"
                                                        ],
                                                        text: "<p class=\"brz-fs-sm-18 brz-fw-lg-400 brz-ff-average_sans brz-ls-lg-0 brz-lh-lg-1_7 brz-text-lg-center brz-fs-lg-15 brz-mt-lg-10 brz-mb-lg-10 brz-fs-xs-18\"><span style=\"opacity: 0.59; color: rgb(0, 6, 7);\">Final Prices As Marked. Only Select Items Apply As Stock Lasts</span></p>"
                                                    }
                                                }
                                            ],
                                            mobilePaddingType: "ungrouped",
                                            mobilePaddingRight: 28,
                                            mobilePaddingRightSuffix: "px",
                                            mobilePadding: 0,
                                            mobilePaddingLeft: 27,
                                            mobilePaddingLeftSuffix: "px"
                                        }
                                    }
                                ],
                                width: 100,
                                paddingTop: 25,
                                paddingTopSuffix: "px",
                                padding: 15,
                                paddingBottom: 20,
                                paddingBottomSuffix: "px",
                                bgColorPalette: "",
                                bgColorHex: "#0bb9c0",
                                bgColorOpacity: 1,
                                tabletPaddingRight: 10,
                                tabletPaddingLeft: 10,
                                mobilePaddingRight: 10,
                                mobilePaddingLeft: 10,
                                borderColorHex: "#ffffff",
                                borderColorOpacity: 1,
                                borderColorPalette: "",
                                tempBorderColorOpacity: 1,
                                borderWidth: 10,
                                borderTopWidth: 10,
                                borderRightWidth: 10,
                                borderBottomWidth: 10,
                                borderLeftWidth: 10,
                                borderRadius: 10,
                                borderTopLeftRadius: 10,
                                borderTopRightRadius: 10,
                                borderBottomRightRadius: 10,
                                borderBottomLeftRadius: 10,
                                tempBorderRadius: 0,
                                tempBorderTopLeftRadius: 0,
                                tempBorderTopRightRadius: 0,
                                tempBorderBottomRightRadius: 0,
                                tempBorderBottomLeftRadius: 0,
                                tempBorderWidth: 4,
                                tempBorderTopWidth: 4,
                                tempBorderRightWidth: 4,
                                tempBorderBottomWidth: 4,
                                tempBorderLeftWidth: 4,
                                boxShadow: "on",
                                boxShadowBlur: 41,
                                boxShadowColorOpacity: 1,
                                boxShadowSpread: -26,
                                boxShadowVertical: 36,
                                tabletPaddingTop: 20,
                                tabletPaddingTopSuffix: "px",
                                tabletPadding: 15,
                                tabletPaddingBottom: 20,
                                tabletPaddingBottomSuffix: "px",
                                tabletMarginRight: 40,
                                tabletMarginRightSuffix: "px",
                                tabletMargin: 0,
                                tabletMarginLeft: 40,
                                tabletMarginLeftSuffix: "px",
                                mobilePaddingTop: 20,
                                mobilePaddingTopSuffix: "px",
                                mobilePadding: 0,
                                mobilePaddingBottom: 20,
                                mobilePaddingBottomSuffix: "px"
                            }
                        }
                    ],
                    bgColorPalette: "",
                    bgColorHex: "#0bb9c0",
                    bgColorOpacity: 0,
                    mobileBgColorHex: "#000000",
                    mobileBgColorOpacity: 0,
                    mobileBgColorPalette: "",
                    mobilePaddingRight: 0,
                    mobilePaddingLeft: 0,
                    borderRadiusType: "grouped",
                    borderTopLeftRadius: 0,
                    borderTopRightRadius: 0,
                    borderBottomLeftRadius: 0,
                    borderBottomRightRadius: 0,
                    borderRadius: 0,
                    tempBorderTopLeftRadius: 10,
                    tempBorderRadius: 0,
                    tempBorderBottomRightRadius: 10,
                    tempBorderTopRightRadius: 10,
                    tempBorderBottomLeftRadius: 10,
                    tempBgColorOpacity: 1,
                    borderColorHex: "#ffffff",
                    borderColorOpacity: 0,
                    borderColorPalette: "",
                    tempBorderColorOpacity: 1,
                    borderWidth: 0,
                    borderTopWidth: 0,
                    borderRightWidth: 0,
                    borderBottomWidth: 0,
                    borderLeftWidth: 0,
                    tempBorderWidth: 8,
                    tempBorderTopWidth: 8,
                    tempBorderRightWidth: 8,
                    tempBorderBottomWidth: 8,
                    tempBorderLeftWidth: 8,
                    tabletPaddingRight: 0,
                    tabletPaddingLeft: 0
                }
            }
        ],
        _blockVisibility: "unlisted",
        bgImageWidth: 1920,
        bgImageHeight: 1281,
        bgImageSrc: "e299aa3097f4fba0829ff206b8fb13aacce12344.jpeg",
        bgPositionX: 50,
        bgPositionY: 50,
        bgColorOpacity: 0.90000000000000002220446049250313080847263336181640625,
        tempBgColorOpacity: 0.90000000000000002220446049250313080847263336181640625,
        mobileBgImageWidth: 1920,
        mobileBgImageHeight: 1281,
        mobileBgImageSrc: "e299aa3097f4fba0829ff206b8fb13aacce12344.jpeg",
        mobileBgPositionX: 50,
        mobileBgPositionY: 50,
        mobileBgColorOpacity: 0.90000000000000002220446049250313080847263336181640625,
        tempMobileBgColorOpacity: 1,
        bgColorHex: "#1b2321",
        bgColorPalette: "",
        mobileBgColorHex: "#1b2321",
        mobileBgColorPalette: "",
        containerSize: 70
    }
}
};